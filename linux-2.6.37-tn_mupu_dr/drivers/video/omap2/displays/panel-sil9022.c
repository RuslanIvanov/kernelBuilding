...
hdmi_enable(struct omap_dss_device *dssdev)
{
...
#if 0/*This part is current not useful, because it might be inconsist to omapfb */
	...
	val = TPI_AVI_PIXEL_REP_TCLKSEL_1X_CLKOUT |
	...
	err = sil9022_read_reg(sil9022_client, 1, SI9022_REG_INPUT_FORMAT, &val);
		dev_info(&sil9022_client->dev, "<%s> i2c Read at 0x%x, bytes value = %x\n",
		 	__func__, SI9022_REG_INPUT_FORMAT, val);	

	err = sil9022_read_reg(sil9022_client, 1, SI9022_REG_OUTPUT_FORMAT, &val);
		dev_info(&sil9022_client->dev, "<%s> i2c Read at 0x%x, bytes value = %x\n",
		 	__func__, SI9022_REG_OUTPUT_FORMAT, val);	
	return 0;
...
#ifdef CONFIG_PM/*platforma_data might be NULL*/
if(dssdev->dev.platform_data!= NULL)
{

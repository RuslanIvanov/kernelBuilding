...
MODULE_LICENSE("GPL");
MODULE_VERSION(PVV3_DRV_VERSION);
MODULE_ALIAS("chdriver:pvv3");
...
static irqreturn_t pvv_irqhandler(int irq, void *dev_id)
{
	...
	irqN  = irq;
	wakeUpUserMode(IRQ_PVV3);

	if(count==0)
	{
		printk(KERN_INFO "MPVV3: IRQ_PVV3 irq %d\n",irq);
		count =1;
	}
...
static long pvv_ioctl(struct file *file,unsigned int cmd,unsigned long arg)
{
	...
	*/
	...
	if (_IOC_TYPE(cmd) != PVV3_IOC_MAGIC) return -ENOTTY;
	if (_IOC_NR(cmd) > PVV3_IOC_MAXNR) return -ENOTTY;
	...
	{        	
		...
		break;		
		case PVV3_IOCG_IRQ_DPU:
		{
			typeIrq=0;

			if(wait_event_interruptible(wqdpu, flagdpu != 0)>0)
    			{
        			printk(KERN_ERR "MPVV3: PVV3_IOCG_IRQ_DPU error ERESTARTSYS");
        			return -ERESTARTSYS;
    			}
			...
			mutex_unlock(&mutex);
                	...
                	if(rez)
                	{
                        	printk(KERN_ERR "MPVV3: PVV3_IOCG_IRQ_DPU error copy_to_user witch IOCTL");
                        	return -EFAULT;
                	}
...
static int pvv_open(struct inode *inode, struct file *pFile)
{
	...
	Device_Status++;
    	...
    	printk(KERN_INFO "MPVV3: pvv_open, count %d\n",Device_Status);
...
{//close
	...
	if(Device_Status>0)
	{
		...
		printk(KERN_INFO "MPVV3: pvv_release, count %d\n",Device_Status+1);
	}
...
{//ssize_t - целое,
	...
	if(wait_event_interruptible(wqdpu, flagdpu != 0)>0)
    	{
        	printk(KERN_ERR "MPVV3: Error ERESTARTSYS");
        	return -ERESTARTSYS;
    	}
...
static int pvv_init(void)
{
	...
	/*err = alloc_chrdev_region(&dev_pvv, 0, COUNT_DEVICES, PVV3_CHDEVNAME);
    	if (err < 0)
    	{
        	printk(KERN_ALERT "MPVV3: alloc_chrdev_region() error %d\n", err);
        	return FAIL;
	}
	...
	printk(KERN_INFO  "MPVV3: Major,Minor numbers [%d,%d]",Major,Minor);
...
//--------------------------------------------------------------------------------------------------------------
	...
	rez = gpio_is_valid(GPIO_PVV3);
	if(rez == -EINVAL) printk(KERN_ERR "not valid pin 27 (PVV3)");
	if(!rez)  printk(KERN_ERR "not valid pin 27 (PVV3)");
	...
	printk(KERN_INFO "------------------------2-----------------------");
    	...
    	if (!pvvdev)
    	{
        	printk(KERN_ERR "MPVV3: could not allocate device for pvv3");
        	unregister_chrdev_region(dev_pvv, COUNT_DEVICES);
	...
	retp = gpio_request(GPIO_PVV3, "irq_pvv3");
    	if (retp < 0)
    	{
        	unregister_chrdev_region(dev_pvv, COUNT_DEVICES);
        	cdev_del(&pvv_cdev->cdev);
	        printk(KERN_INFO "error gpio_request(%d) error = %d",GPIO_PVV3,retp);
        	return -1;
    	}

	retp = gpio_direction_input(GPIO_PVV3);
   	if (retp < 0)
    	{
        	unregister_chrdev_region(dev_pvv, COUNT_DEVICES);
        	cdev_del(&pvv_cdev->cdev);
	        printk(KERN_INFO "error gpio_direction_input(%d) error = %d",GPIO_PVV3,retp);
        	return -1;
    	}

	irq_pvv3 = -1;
	if(retp == 0) {gpio_export(GPIO_PVV3,0); irq_pvv3 = OMAP_GPIO_IRQ(GPIO_PVV3);}

	//irq = gpio_to_irq(GPIO_PVV3);
	if (irq_pvv3 < 0) 
	{
        	unregister_chrdev_region(dev_pvv, COUNT_DEVICES);
        	cdev_del(&pvv_cdev->cdev);
	...
	pvvdev->irq = irq_pvv3;
	printk(KERN_INFO "irq_pvv3: %d",irq_pvv3);
	pvvdev->idrev = pvvdev;
  	pvvdev->ioaddr = 0;

    	strcpy(pvvdev->name,"pvv3");
...
{
	gpio_free(GPIO_PVV3);
	free_irq(pvvdev->irq,GPIO_PVV3_DEVICE_DESC);
	kfree(pvvdev);
    	unregister_chrdev_region(dev_pvv,COUNT_DEVICES);
	cdev_del(&pvv_cdev->cdev);

	printk(KERN_INFO "MPVV3: STOP");
}

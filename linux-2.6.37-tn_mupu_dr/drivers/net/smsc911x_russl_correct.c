...
static int smsc911x_soft_reset(struct smsc911x_data *pdata)
{
	...
	printk(KERN_INFO "smsc911x_init: smsc911x_soft_reset:  HW_CFG %x",temp);
...
static int __devinit smsc911x_init(struct net_device *dev)
{
	...
	printk(KERN_INFO "smsc911x_init: id smsc %x",pdata->idrev & 0xFFFF0000);
	switch (pdata->idrev & 0xFFFF0000) {
	case 0x01180000:
	case 0x01170000:
	case 0x01160000:
	...
	if (pdata->config.flags & SMSC911X_SAVE_MAC_ADDRESS)
    	...
    	printk(KERN_INFO "smsc911x_init: smsc911x_soft_reset");
	...
	printk(KERN_INFO "smsc911x_init: Disable all interrupt sources until we bring the device up");
	...
	printk(KERN_INFO "smsc911x_init: end init");
...
static int __devinit smsc911x_drv_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "smsc911x: probe");
	...
	printk(KERN_INFO "smsc911x: res->start [%p,%p]",res->start,res->end);
	...
	printk(KERN_INFO "smsc911x: Base addr: %x [%p]", pdata->ioaddr,  pdata->ioaddr);
...
late_initcall(smsc911x_init_module);

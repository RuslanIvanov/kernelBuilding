...
#include "cmd.h"
#include "if_sdio.h"
#if defined(CONFIG_LIBERTAS_SDIO_FIRMWARE)
#include "sd8686_helper_bin.h"
#include "sd8686_bin.h"
#endif
				...
				const struct firmware *fw)
{
	...
	#if defined(CONFIG_LIBERTAS_SDIO_FIRMWARE)
	firmware = sd8686_helper_bin;
	size = sizeof(sd8686_helper_bin);
	#else
	firmware = fw->data;
	size = fw->size;
	#endif
...
static int if_sdio_prog_firmware(struct if_sdio_card *card)
{
	...
	#if !defined(CONFIG_LIBERTAS_SDIO_FIRMWARE)
	ret = request_firmware(&helper, card->helper, &card->func->dev);
	if (ret) {
		lbs_pr_err("can't load helper firmware\n");
		goto out;
	}
...
static int __init if_sdio_init_module(void)
{
	...
	omap_hsmmc_inform_detection(0);
	msleep(100);
	omap_hsmmc_inform_detection(1);
#endif
